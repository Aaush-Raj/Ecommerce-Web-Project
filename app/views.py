...
def show_cart(request):
    if request.user.is_authenticated:
        ...
        total_amount = 0.0
        cart_product = [p for p in Cart.objects.all() if p.user == user] #this is LIST COMPREHENSION!
        #print(cart_product)
        if cart_product:
...
def plus_cart(request):
    if request.method == 'GET':
        ...
        amount = 0.0
        shipping_amount = 50.0
        cart_product = [p for p in Cart.objects.all() if p.user == request.user] #this is LIST COMPREHENSION!
        for p in cart_product:
            tempamount = (p.quantity * p.product.discounted_price)
            amount += tempamount
...
@login_required
def address(request):
    add = Customer.objects.filter(user=request.user)#current user is located at - request.user
...
def topwear(request,data = None):
 ...
 #   elif data == 'Apple' or data == 'Dell':
    #    laptop = Product.objects.filter(category='TW').filter(brand=data)
...
def bottomwear(request,data = None):
   ...
   # elif data == 'Apple' or data == 'Dell':
    #    laptop = Product.objects.filter(category='BW').filter(brand=data)
